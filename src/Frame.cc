     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    outfile.open("keypoints.txt", std::ios_base::app);   // append instead of overwrite
    outfile << (unsigned long long)(timeStamp * pow(10, 9)) << " " << N << '\n'; 

    // Muchang's edit
    
    /* std::cout << (unsigned long long)(timeStamp * pow(10, 9)) << " " << N << std::endl;  */
